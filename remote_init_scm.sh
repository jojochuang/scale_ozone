...
ssh $SSH_PASSWORDLESS_USER@$SCM_HOST$CLUSTER_DOMAIN sudo mkdir $SCM_DIR
ssh $SSH_PASSWORDLESS_USER@$SCM_HOST$CLUSTER_DOMAIN sudo chmod 755 $SCM_DIR
ssh $SSH_PASSWORDLESS_USER@$SCM_HOST$CLUSTER_DOMAIN sudo chown -R hdfs:hdfs $SCM_DIR
